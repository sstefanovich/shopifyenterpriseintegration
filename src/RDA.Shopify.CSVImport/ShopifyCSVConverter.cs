...
namespace RDA.Shopify.CSVImport
{
    public class ShopifyCSVConverter : ICSVConverter
    {
        ...
        private string BuildProduct(Product product, bool trackLocationInventory)
        {
            ...
            if (product.Title.IndexOfAny(quotableChars) != -1)
                productRecord.AppendQuoted(product.Title.Replace("\"", "\"\""));
            else
                productRecord.Append(product.Title.Replace("\"", "\"\""));
            productRecord.Append(",");

            productRecord.AppendQuoted(product.Body.Replace("\"", "\"\""));
            productRecord.Append(",");

            productRecord.Append(product.Vendor);
            productRecord.Append(",");

            if (product.Type.IndexOfAny(quotableChars) != -1)
                productRecord.AppendQuoted(product.Type);
            else
